...
- **Engine-follower**: Only one process holds the search index in RAM. Additional processes proxy to it via localhost HTTP (port 19742 by default).
...
| `search.py` | SearchIndex (in-memory numpy matrix), hybrid search routing (engine/follower), peer fan-out |
...
| `coordination.py` | File-lock based engine election, HTTP server for followers, failover |
...
- **search.py extraction**: SearchIndex and all search routing logic (engine/follower/peer fan-out) live in `search.py`, keeping `server.py` focused solely on MCP tool definitions and initialization
